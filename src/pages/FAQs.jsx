  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-16">
      <div className="text-center mb-10">
        <h1 className="text-3xl md:text-5xl font-extrabold text-white mb-4">How can we help you today?</h1>
        ...
        <div className="relative max-w-lg mx-auto">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl pl-11 pr-4 py-3 text-sm outline-none focus:border-primary text-white transition-colors"
          />
        </div>
      </div>
          ...
          {categories.map((c) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-left transition-colors ${
                active === c.key ? "bg-white/10 text-primary border border-white/5" : "text-gray-400 hover:bg-white/5 border border-transparent"
              }`}
            >
        ...
        <div className="md:col-span-3">
          ...
          <div className="space-y-3 mb-10">
            {section.items
              ...
              .map((f, i) => (
                <div key={f.q} className="border border-white/10 bg-white/5 rounded-xl overflow-hidden hover:bg-white/10 transition-colors">
                  <button
                    onClick={() => setOpenIdx(openIdx === i ? null : i)}
                    className="w-full flex items-center justify-between px-5 py-4 text-left font-medium text-white"
                  >
                    {f.q}
                    <ChevronDown
                      size={18}
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-3 shrink-0">
              <button className="flex items-center gap-2 bg-primary text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-primary/90 transition-colors">
                <MessageCircle size={16} /> Chat with Us
              </button>
              <button className="flex items-center gap-2 border border-white/20 text-gray-300 hover:text-white hover:bg-white/5 text-sm font-semibold px-4 py-2.5 rounded-lg transition-colors">
